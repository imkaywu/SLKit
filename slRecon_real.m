...
obj_name     = 'man';    % object name (should correspond to a data dir.)
...
for icam = 1:ncams
   ...
   I = imread(sprintf('%s/0000.bmp', data_dir));
...
for icam = 1:ncams
   ...
   % first two are allone, allzero
   img_pattern_ref{1}{icam} = imread(sprintf('%s/%04d.bmp', data_dir, 0));
   img_pattern_ref{2}{icam} = imread(sprintf('%s/%04d.bmp', data_dir, 1));
   frameIdx = 2;
   for i = 1:2 % column/row pattern
      for j = 1:npatterns{icam} % index of bitplane
         img_pattern1{i, j}{icam} = imread(sprintf('%s/%04d.bmp', data_dir, frameIdx));
         frameIdx = frameIdx + 1;
         img_pattern2{i, j}{icam} = imread(sprintf('%s/%04d.bmp', data_dir, frameIdx));
         frameIdx = frameIdx + 1;
      end
   end
end
...
for i = 1:length(Nc)
   ...
   vV = intersectLineWithPlane(repmat(Oc{i},1,npts),Nc{i}(:,idx),wPlaneCol(est_code{1,i}(idx),:)');
   vH = intersectLineWithPlane(repmat(Oc{i},1,npts),Nc{i}(:,idx),wPlaneRow(est_code{2,i}(idx),:)');
   vertices{i} = vV';
